...
class PatternSeekerCooperateTimesGoldstein(Agent):
    ...
    class AnalyseSymmetryBehav(OneShotBehaviour):
        ...
        async def run(self):
            ...
            ac1ac2 = await self.calculate_cooperate_times_goldstein_percentage(actor1, actor2)
            ac2ac1 = await self.calculate_cooperate_times_goldstein_percentage(actor2, actor1)

            symmetry = ac1ac2.append(ac2ac1)
            create_cooperate_times_goldstein_figure(symmetry, actor1, actor2)

            self.agent.symmetry = symmetry

            self.agent.add_behaviour(self.agent.SendResultsBehav())

        async def calculate_cooperate_times_goldstein_percentage(self, actor1, actor2):
            print(f"{self.agent.jid}: {self.__class__.__name__}: Calculating cooperate percentage {actor1}-{actor2}")
            QUERY = (f"""SELECT
  MonthYear,
  COUNT(*) AS Count,
  SUM(GoldsteinScale) AS SumGoldsteinScale,
  Actor2CountryCode
FROM
...
GROUP BY
  ...
  Actor2CountryCode""")
            ...
            s = ac1ac2monthyear.groupby(["Time"]).agg({'SumGoldsteinScale': 'sum'})
            t = ac1monthyear.groupby(["Time"]).agg({'SumGoldsteinScale': 'sum'})
            s['Percentage'] = s['SumGoldsteinScale'] / t['SumGoldsteinScale'] * 100
