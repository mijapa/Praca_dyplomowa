...
from spade_proto.auxiliary import authenticate_google_cloud, perform_query, string_to_list, create_symmetry_figure, \
    create_fight_figure, create_fight_vs_all_figure


class PatternSeekerFightVsAll(Agent):
    config = []
    ...
    class SendResultsBehav(OneShotBehaviour):
        async def run(self):
            ...
            msg.set_metadata("ontology", "fight_results")  # Set the ontology of the message content
    ...
    class AnalyseSymmetryBehav(OneShotBehaviour):
        ...
        async def run(self):
            ...
            ac1ac2 = await self.calculate_fight_percentage(actor1, actor2)
            ac2ac1 = await self.calculate_fight_percentage(actor2, actor1)

            symmetry = ac1ac2.append(ac2ac1)
            create_fight_vs_all_figure(symmetry, actor1, actor2)

            self.agent.symmetry = symmetry

            self.agent.add_behaviour(self.agent.SendResultsBehav())

        async def calculate_fight_percentage(self, actor1, actor2):
            print(f"{self.agent.jid}: {self.__class__.__name__}: Calculating fight vs all percentage {actor1}-{actor2}")
            QUERY = (f"""SELECT
  MonthYear,
  COUNT(*) AS Count,
  Actor2CountryCode,
