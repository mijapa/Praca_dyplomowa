...
class PatternSeekerCooperateTimesNumMen(Agent):
    ...
    class AnalyseSymmetryBehav(OneShotBehaviour):
        ...
        async def run(self):
            ...
            ac1ac2 = await self.calculate_cooperate_times_nummen_percentage(actor1, actor2)
            ac2ac1 = await self.calculate_cooperate_times_nummen_percentage(actor2, actor1)

            symmetry = ac1ac2.append(ac2ac1)
            create_cooperate_times_nummen_figure(symmetry, actor1, actor2)

            self.agent.symmetry = symmetry

            self.agent.add_behaviour(self.agent.SendResultsBehav())

        async def calculate_cooperate_times_nummen_percentage(self, actor1, actor2):
...
GROUP BY
  ...
  Actor2CountryCode""")
            ...
            s = ac1ac2monthyear.groupby(["Time"]).agg({'SumNumMentions': 'sum'})
            t = ac1monthyear.groupby(["Time"]).agg({'SumNumMentions': 'sum'})
            s['Percentage'] = s['SumNumMentions'] / t['SumNumMentions'] * 100
