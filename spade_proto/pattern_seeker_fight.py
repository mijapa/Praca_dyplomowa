...
class PatternSeekerFight(Agent):
    config = []
    ...
    class AnalyseSymmetryBehav(OneShotBehaviour):
        ...
        async def run(self):
            ...
            create_fight_figure(symmetry, actor1, actor2)
        ...
        async def calculate_fight_percentage(self, actor1, actor2):
            print(f"{self.agent.jid}: {self.__class__.__name__}: Calculating fight percentage {actor1}-{actor2}")
            QUERY = (f"""SELECT
  MonthYear,
  COUNT(*) AS Count,
  Actor2CountryCode
